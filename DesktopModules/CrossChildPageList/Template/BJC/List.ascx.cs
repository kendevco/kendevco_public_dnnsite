...
namespace Cross.Modules.ChildPageList
{
	...
	partial class List : PortalModuleBase
	{
		...
		protected string RenderTabItem(TabInfo tabInfo)
		{
            ...
            //Phuoc added
            string genURL = string.Empty;
            if (tabInfo.Url.Contains("FileID"))//File
            {
                int fileId = int.Parse(tabInfo.Url.Substring(7));

                DotNetNuke.Services.FileSystem.IFileInfo fi = FileManager.Instance.GetFile(fileId);//DNN 7

                genURL = PortalSettings.HomeDirectory + fi.Folder + fi.FileName;// tabInfo.Url;
                //genURL = Globals.LinkClick(tabInfo.Url,TabId,ModuleId);// this doesn't work for authenticated users.
            }
            else if (!isInternalLink)//external urls
            {
                genURL = tabInfo.Url;
            }
            else
            {
                genURL = DotNetNuke.Common.Globals.NavigateURL(tabInfo.TabID);
            }
            //end
			if(genURL.Contains("http://"))
			{
					genURL = genURL.Replace("http", "https");
			}

            string link = String.Format("<a href='{0}' target='{2}'>{1}</a>", genURL,
