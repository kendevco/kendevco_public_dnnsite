            ...
            function ($scope, $http, dnnsfHttp) {
                ...
                var fnSaveKey = function (data, fnDone) {
                    ...
                    var url = $scope.app.SupportsWebAPI
                        ? $scope.app.BaseUrl + '/API/settings/SaveActivation?_aliasid=' + encodeURIComponent(window.currentAliasId)
                        : $scope.app.BaseUrl + '/AdminApi.ashx?Method=SaveActivation&_aliasid=' + encodeURIComponent(window.currentAliasId);
                    var qsMid = dnnsf.urlParam('_mid');
                    if (qsMid == undefined || qsMid == "") {
                        qsMid = -1
                    }
