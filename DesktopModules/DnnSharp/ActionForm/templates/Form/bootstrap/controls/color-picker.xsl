  ...
  <xsl:template name="ctl-color-picker">
    ...
    <div>
      ...
      <div load-on-demand="'colorpicker'">
        <input type="text" colorpicker="" color-picker-position="bottom" data-inputmask-alias="colorpicker" update-field="updateField(field, val)">
          ...
          <xsl:attribute name="color-picker-model">
              form.fields.<xsl:value-of select="Name" />.value
          </xsl:attribute>
