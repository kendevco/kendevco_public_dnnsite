      ...
      <div load-on-demand="'angulartextbox'" >
        ...
        <input type="$type">
          ...
          <xsl:if test="AutocompleteUrl != ''">
            <xsl:attribute name="data-autocomplete-url">
              <xsl:value-of select="AutocompleteUrl"/>
            </xsl:attribute>
          </xsl:if>

          <xsl:if test="$type = 'number'">
			<xsl:attribute name="enforce-min-max-number"></xsl:attribute>
            <xsl:attribute name="max-number">
              <xsl:value-of select="MaxValue"/>
            </xsl:attribute>
            <xsl:attribute name="min-number">
              <xsl:value-of select="MinValue"/>
            </xsl:attribute>
		    <xsl:attribute name="ng-blur">
		      <xsl:text>enforceMinMaxOnBlur($event.target.value)</xsl:text>
		    </xsl:attribute>
          </xsl:if>
